...
@php
    if($type == "jobs") {
        $elements = App\Constants::JOB_SORTING;
        $route = route('jobs.sorting');
        $session = Session::get('job-sorting', 'newest');
    } elseif($type == "employers") {
        $elements = App\Constants::EMPLOYER_SORTING;
        $route = route('employers.sorting');
        $session = Session::get('employer-sorting', 'alpha_asc');
    } else {
